...
bool player_2_exists(int row, int col) {
	return row >= 0 && row < SIZE && col >= 0 && col < SIZE;
}

void player_2_bfs(gamestate &gms, matrix &dist, vector<coord> &twrs) {
...
LD player_2_evaluate_capturing(gamestate &gms, matrix &dist, vector<coord> &twrs) {
	LD res = 0;
	int distsum = 0;
...
LD player_2_evaluate(gamestate &gms, const tmove &move) {
	LD ans = 0;
	if (gms.isolated) {
		...
		player_2_bfs(tgms, dist_op, tgms.op.twrs);
		player_2_bfs(tgms, dist_me, tgms.me.twrs);

		ans += player_2_evaluate_capturing(tgms, dist_me, tgms.me.twrs);
		ans -= player_2_evaluate_capturing(tgms, dist_op, tgms.op.twrs);
	}
